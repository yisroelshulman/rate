...
func main() {
	...
	*/
	...
	limit := NewBufferedLimiter(1, 6, time.Second*2)
...
func lineup(lim *BufferedLimiter, t *time.Duration, thread int) {
	res.mu.Lock()
	...
	if err != nil {
		res.mu.Lock()
		res.s = append(res.s, fmt.Sprintf("error from thread %d at %v: %v\n", thread, time.Now(), err))
		res.mu.Unlock()
		return
	}
	res.mu.Lock()
	res.s = append(res.s, fmt.Sprintf("processed request from thread %d at %v\n", thread, time.Now()))
	res.mu.Unlock()
}
